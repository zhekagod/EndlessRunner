...
ARunCharacter::ARunCharacter()
{
	...
	CameraArm->TargetArmLength = 350.f; // Дистанция от камеры до персонажа
	CameraArm->SocketOffset = FVector(0.f, 0.f, 100.f); // Начальное положение камеры
	CameraArm->bUsePawnControlRotation = true; // Разрешить вращать камеру по окружности, как голову
	CameraArm->SetupAttachment(GetRootComponent()); // Привязываем камеру к одной точке


	Camera = CreateDefaultSubobject<UCameraComponent>(TEXT("Camer"));
	Camera->bUsePawnControlRotation = false;
	Camera->SetupAttachment(CameraArm, USpringArmComponent::SocketName);//Привязка камеры к персонажу
	//Ctrl+Shift+B
}
...
void ARunCharacter::Tick(float DeltaTime)
{
	...
	FRotator ControlRot = GetControlRotation(); //Получаем позицию поворота персонажа
	ControlRot.Roll = 0.f;
	ControlRot.Pitch = 0.f;

	AddMovementInput(ControlRot.Vector()); //Ввод с клавы с соответствующим направлением
...
{
	NextLane = FMath::Clamp(CurrentLane - 1, 0, 2); //Смена полосы без выхода за границы (зажим)
	ChangeLane();
	UE_LOG(LogTemp, Warning, TEXT("MOVE LEFT WAS PRESSED")); //отладка
}
