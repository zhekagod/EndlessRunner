...
//вывод монет на экран и зануление при старте
void UGameHud::InitializeHUD(AEndlessRunnerGameModeBase* RunGameMode)
{
	if (RunGameMode)
	{
		CoinsCount->SetText(FText::AsNumber(0));
		LivesCount->SetText(FText::AsNumber(RunGameMode->MaxLives)); // Ошибка E0109 выражение перед скобками вероятного вызова должно иметь тип функции(pointer - to - )	
